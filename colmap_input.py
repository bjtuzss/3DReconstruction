...
if __name__ == '__main__':
    ...
    parser.add_argument('--convert_images_name', action='store_true', default=False, help='If set, convert image name to MVSNet format.')
    ...
    for i, j in tqdm(queue):
        ...
        for pid in id_intersect:
            ...
            p = points3d[pid].xyz
            theta = (180 / np.pi) * np.arccos(np.dot(cam_center_i - p, cam_center_j - p) / np.linalg.norm(cam_center_i - p) / np.linalg.norm(cam_center_j - p))
            s += np.exp(-(theta - args.theta0) * (theta - args.theta0) / (2 * (args.sigma1 if theta <= args.theta0 else args.sigma2) ** 2))
        score[i, j] = s
        score[j, i] = s
