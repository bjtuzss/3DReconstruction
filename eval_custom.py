...
from cal_norm import cal_pc_norm
import math
...
parser.add_argument('--dataset', default='custom', help='select dataset')
parser.add_argument('--testpath', default='./data/scan4-short', help='testing data path')
...
# cpu:model_cpu_m.ckpt gpu:model_000007.ckpt
# parser.add_argument('--loadckpt', default='./checkpoints/model_cpu_m.ckpt', help='load a specific checkpoint')
parser.add_argument('--loadckpt', default='./checkpoints/model_000007.ckpt', help='load a specific checkpoint')
parser.add_argument('--outdir', default='./outputs/scan4-short', help='output dir')
parser.add_argument('--outfile_name', default='scan4-short.obj', help='output dir')
...
# threshold_origin: geo_pixel_thres:1;geo_depth_thres:0.01;photo_thres:0.8
...
parser.add_argument('--photo_thres', type=float, default=0.9, help='threshold for photometric consistency filtering')
...
def save_depth():
    ...
    test_dataset = MVSDataset(args.testpath, args.n_views, (args.imw, args.imh))
...
depth=12  # Maximum depth of the tree that will be used for surface reconstruction
'''
def possion_restruction(path,radius = 0.001, max_nn = 30, depth=8):
...
if __name__ == '__main__':
    ...
    # step3. point cloud 2 mesh            -new
    mesh = possion_restruction(os.path.join(args.outdir, 'custom.ply'))
    print(type(mesh))
    # o3d.visualization.draw_geometries([mesh])
    o3d.io.write_triangle_mesh(os.path.join(args.outdir,args.outfile_name),mesh)
    # step3. point cloud 2 mesh            -old
    # data = read_ply(os.path.join(args.outdir, 'custom.ply'))
    # data['normal'] = cal_pc_norm(os.path.join(args.outdir, 'custom.ply'))
    # save_ply(data['point'], os.path.join(args.outdir, 'custom_normal.ply'), data['color'], data['normal'])
    # os.system("PoissonRecon.exe --in " + os.path.join(args.outdir, 'custom_normal.ply') + " --out " + os.path.join(
    #     args.outdir, "custom-meshed.ply"))
    # ply2obj(os.path.join(args.outdir, 'custom-meshed.ply'), os.path.join(args.outdir, "custom-meshed.obj"))